/* Hasan YENİADA_220201024, İsmail Ulaş BAYRAM_220201040 */

/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>  MAIN PART  >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/	
main(X, Y) :-																			
	integer(X),integer(Y),
	Counter is 1, ((X > 0, Y > 0);(X = 0,not(Y = 0))),    /* main rule for positive X and positive Y or X = 0 and Y is not zero */
	extended(X, Y, GCD, X, Y, P, S, _, _, _, _, Counter),	/* Note: We cannot apply extended euclidian algorithm to X is an integer and Y = 0 condition!! */
	write('P,S and GCD values of given numbers are at below: '),nl,
	write('P = '), write(P),nl,write('S = '),write(S),nl,write('GCD = '),write(GCD).

main(X, Y) :-
	integer(X),integer(Y),
	Counter is 1, X < 0, Y > 0,		/* main rule for negative x and positive y */
	New_X is -1 * X,
...
main(X, Y) :-
	integer(X),integer(Y),
	Counter is 1, X > 0, Y < 0,		/* main rule for positive x and negative y */
...
main(X, Y) :-
	integer(X),integer(Y),
	Counter is 1, X < 0, Y < 0,		/* main rule for negative x and negative y */
...
extended(First, Second, GCD, X, Y, P, S, P0, P1, Q0, _, _) :-   /* base case for (positive x and y) or (negative x positive y) */
...
extended(First, Second, GCD, X, Y, P, S, P0, P1, Q0, _, _) :-   /* base case for NEGATIVE Y AND POSITIVE X  or BOTH of THEM NEGATIVE */
	Y = 0, not(X = Second),((Second < 0, First > 0);(First < 0, Second < 0)), 	/* helper3 calculates minus s and then according to formula: */
	helper2(First, Second, GCD, X, P, S, P0, P1, Q0).							/*	pX + sY = gcd(X,Y) calculates related P */
																				
/*>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>> ExTENDED EUCLIDIAN OPERATIONS PART >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>*/
extended(First, Second, GCD, X, Y, P, S, P0, P1, Q0, Q1, Counter) :-  /* first step */
...
extended(First, Second, GCD, X, Y, P, S, P0, P1, Q0, Q1, Counter) :-   /* second step */
	not(Y = 0),  Counter = 2, Counter2 is 3,
	P1 is 1, 	/* at the first loop, p1 = 1 and we are passing to third step and we know p0 and p1, at every step these p0 and p1 will be updated according to formula */
	Q1 is X // Y,
	New_Y is mod(X,Y), 
